...
/**
 * INFORMATION ABOUT THE SESSION SHOWN WHEN VIEWING A PARTICULAR GROUP SESSION
 */
  ...
  return (
    <div>
      ...
      <p>
        ...
        {limitedspacetext2}
        {limited ? (
          <ProgressBar // progress bar for the registrations
            visuallyHidden={true}
            min={0}
            max={space}
            now={spotsTaken}
            animated={true}
            label={`${spotsTaken} of ${space}`}
          />
        ) : (
          ""
        )}
      ...
      <p>
        {" "}
        <h6>The price for this session is:</h6> {free ? "free session" : price + " euros"}{" "}
      </p>
      <p>
        {" "}
        <h6>The session will take place in the following location:</h6> {location}
      </p>
    </div>
  );
}
